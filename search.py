...
def uniformCostSearch(problem):
    ...
    while not open.isEmpty():
        ...
        for successor, action, g in problem.getSuccessors(state):
            if successor not in closed:
            # if no succ or succ are all in closed,
            # then this loop ends and starts next loop(pop a new node)
                open.push(
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not open.isEmpty():
        ...
        #not in goal state, push every unvisited state into
        for successor, action , g in problem.getSuccessors(state):
            if successor not in closed:
                open.push(
                #node = (state,[actions])
                    (successor, actions + [action]),
                    # g + h + (totalcost, to prevent detour)
                    g + heuristic(successor, problem = problem) +
